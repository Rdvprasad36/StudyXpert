...
export async function POST(request: Request) {
  try {
    ...
    if (provider === 'exa') {
      ...
      // Temporarily remove dateRestrict and safe parameters to isolate error
      // const dateRestrict = getGoogleDateRestrict(timeFilter as TimeFilter)
      // if (dateRestrict) {
      //   params.append('dateRestrict', dateRestrict)
      // }

      // const googleSafeSearchMap: Record<string, string> = {
      //   'off': 'off',
      //   'active': 'medium', // map 'active' to 'medium' for Google API
      // }
      // const safeSearchValue = googleSafeSearchMap[CONFIG.search.safeSearch.google] || 'off'
      // params.append('safe', safeSearchValue)
      ...
      if (!googleResponse.ok) {
       ...
       console.error('Google Search API error response:', JSON.stringify(errorData, null, 2))
