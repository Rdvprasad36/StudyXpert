...
import { generateWithGeminiEnhanced } from '@/lib/gemini-enhanced'
...
Make the query clear and focused, avoiding overly complex or lengthy constructions.`
    ...
    try {
      ...
      if (platform === 'google') {
        response = await generateWithGeminiEnhanced(systemPrompt, model)
        modelUsed = platformModel
      } else {
        response = await generateWithModel(systemPrompt, platformModel)
        modelUsed = platformModel
      }
